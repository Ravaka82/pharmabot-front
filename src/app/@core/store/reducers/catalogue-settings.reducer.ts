...
import {CatalogueSettingsActions as ModelActions, AccountActions} from '../actions';
import {createEntityAdapter, EntityAdapter} from "@ngrx/entity";
import * as Model from '../../interfaces/catalogue-settings.interface'
...
export const reducer = createReducer(
  ...
  on(
    ...
    (state, {error: { errorMessage, action}}) => {
      console.log('Catalogue-Settings::failed', errorMessage)
      return {...state, loading: false, errorMessage, action}
    }),
  on(AccountActions.LOGOUT_SUCCEED, _ => {
    console.log('initialize catalogue-settings state.');
