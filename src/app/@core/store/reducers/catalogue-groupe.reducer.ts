...
import {CatalogueGroupeActions as ModelActions, AccountActions} from '../actions';
import {createEntityAdapter, EntityAdapter} from "@ngrx/entity";
import * as Model from '../../interfaces/catalogue-groupe.interface'
...
export const reducer = createReducer(
  ...
  on(
    ...
    (state, {error: { errorMessage, action}}) => {
      console.log('Catalogue-Groupe::failed', errorMessage)
      return {...state, loading: false, errorMessage, action}
    }),
  on (ModelActions.SELECTED, (state, {catalogueGroupeId}) => {
    return { ...state, selected: catalogueGroupeId}
  }),
  on(AccountActions.LOGOUT_SUCCEED, _ => {
    console.log('initialize catalogue-groupe state.');
