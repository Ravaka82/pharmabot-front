...
import {Entry} from "src/app/@core/interfaces/catalogue-import.interface";
import {createReducer, on} from "@ngrx/store";
import {CatalogueImportState as State} from '../states';
import {AccountActions, CatalogueImportActions as ModelActions} from '../actions';
...
export const reducer = createReducer(
  ...
  on(ModelActions.DELETE_SUCCEED, (state, {entry: {_id}}) => {
    return adapter.removeOne(_id, { ...state, loading: false });
  }),
  on(AccountActions.LOGOUT_SUCCEED, _ => {
    console.log('initialize catalogue-import state.');
