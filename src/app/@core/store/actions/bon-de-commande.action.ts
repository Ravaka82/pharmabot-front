...
const source = 'bon-de-commande';

export const LOAD_REQUESTED = createAction(
  `[${source}] load request`,
  props<{to:string,subject:string,text:string,file:any,filename:string}>()
);
...
export const CREATE_REQUESTED = createAction(
  ...
  props<{input: MODEL.AddBonDeCommandeInput}>()
);
...
export const UPDATE_REQUESTED = createAction(
  ...
  props<{id: string, changes: MODEL.UpdateBonDeCommandeInput}>()
);
...
export const DELETE_REQUESTED = createAction(
  ...
  props< {input: MODEL.DeleteBonDeCommandeInput}>()
);
