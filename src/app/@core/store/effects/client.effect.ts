...
import {ClientActions as fromAction, HistoryActions} from "../actions";
        ...
        mergeMap(response => {
          return [
            HistoryActions.CREATE_REQUESTED({input: [{
                action: 'CREATE',
                description: `Un nouveau client '${response.name}' a été créé.`,
      ...
      .pipe(
        mergeMap(response => {
          ...
          return [
            HistoryActions.CREATE_REQUESTED({input: [{
                action: 'UPDATE',
                description: `Le client '${changes.name}' a été mis à jour.`,
                user: GET_ITEM_STORAGE('currentUserId') || '',
                metadata: {
                  input: {id, changes},
                  output: response
      ...
      .pipe(
        mergeMap(response => {
          return [
            HistoryActions.CREATE_REQUESTED({input: [{
                action: 'DELETE',
                description: `Le client '${response.name}' a été supprimé.`,
                user: GET_ITEM_STORAGE('currentUserId') || '',
                metadata: {
                  input: _id,
                  output: response
